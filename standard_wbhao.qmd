...
```{r, load library, eval=FALSE, include=TRUE, message=FALSE}
library(tidyverse)
```

## Recoding for ASEC

```{r, asec race coding, eval=FALSE, include=TRUE, message=FALSE}
mutate(
    wbhao = case_when(
...
```{r, acs race coding 1, eval=FALSE, include=TRUE, message=FALSE}
mutate(
    wbhao = case_when(
...
```{r, acs race coding 2, eval=FALSE, include=TRUE, message=FALSE}
mutate(
    wbhao = case_when(
        hispan %in% c(1,2,3,4) ~ 3, 
        race == 1 ~ 1, 
