---
title: "Right-to-work status by state, over time"
author: "Emma Cohn"
date: "2026-02-10"
...
Some states have adopted or, as in the case of Michigan, repealed right-to-work laws at varying times over the last eighty years. These changes make it difficult to accurately compare economic conditions over time on the basis of a state's right-to-work status.
...
In this chunk, you'll reformat the RTW label **state_rtw_defintions.csv** file into a format that's easier to process in R.

```{r, RTW status CSV, message=FALSE}

# read in RTW status by year
rtw_status_year <- read.csv("./input/state_rtw_defintions.csv") |> 
