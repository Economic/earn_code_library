---
title: "Union wage premiums by state"
author: "Emma Cohn"
format: html
editor: visual
---
...
Running this script chunk will call the BLS Current Population Survey ORG data required to calculate union wage premiums. We filter out wages allocated by the Census/BLS (variables `a_earnhour` and `a_weekpay`) because imputed (estimated by the Census/BLS) wages don't take into account union status. This is problematic when attempting to estimate the differences in wages by union status.
...
cps_org <- load_org(2020:2024, "year", "age", "statefips", "wage", "union", 
                    "orgwgt", "a_earnhour", "a_weekpay", "cow1", "paidhre") |>
