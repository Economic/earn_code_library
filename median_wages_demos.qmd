...
EPI uses the chained CPI-U series to inflation adjust wages (use chained extended for analysis pre-2000), so we'll select that series and assign it to a dataframe.
...
cps_org <- load_cps("org", 1979:2024, year, orgwgt, wage, statefips, female, wbho, age, 
  ...
  mutate(realwage = wage * (cpi2024/c_cpi_u_extended))
```
